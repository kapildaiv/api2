...
def configure_ngrok():
    ngrok_token = "2pKb3XWancRgR6Z0XSDGRVPjbyW_6bHPtXPvM7QnkBK577xYr"
    try:
...
def update_soul_txt(public_url):
    ...
    print(f"New ngrok link saved in sid.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("sid.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated sid.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update sid.txt on VPS: {str(e)}")
